...
"-L <file>     File containing list of addresses to search for\n"
...
main(int argc, char **argv)
{
    ...
    const char *address_list_file = NULL;
    vg_address_list_t *address_list = NULL;

    int i;

    while ((opt = getopt(argc, argv,
                 "vqik1NTX:eE:p:P:d:w:t:g:b:VSh?f:o:s:D:L:")) != -1) {
        switch (opt) {
        case 'v':
            verbose = 2;
            break;
        ...
        case 'L':
            if (address_list_file != NULL) {
                fprintf(stderr,
                    "Multiple address list files specified\n");
                return 1;
            }
            address_list_file = optarg;
            break;
        default:
            usage(argv[0]);
            return 1;
    ...
    if (address_list_file) {
        address_list = vg_load_address_list(address_list_file, verbose);
        if (!address_list) {
            return 1;
        }
        if (!vg_context_add_patterns(vcp,
                         (const char ** const) &address_list_file,
                         1))
            return 1;
    } else {
        if (!npattfp) {
    ...
    if (ndevstrs) {
        for (opt = 0; opt < ndevstrs; opt++) {
            ...
            if (!vocp) {
                ...
                if (address_list) {
                    if (!vg_ocl_context_set_address_list(vocp, address_list)) {
                        fprintf(stderr,
                            "Failed to set address list on device '%s'\n",
                            devstrs[opt]);
        ...
        if (vocp) {
            if (address_list) {
                if (!vg_ocl_context_set_address_list(vocp, address_list)) {
                    fprintf(stderr,
                        "Failed to set address list on device\n");
                    vg_ocl_context_free(vocp);
                    vocp = NULL;
                } else {
                    opened++;
    ...
    if (!opened) {
        ...
        if (address_list)
            vg_free_address_list(address_list);
        return 1;
    }
