...
static int
vg_ocl_addrlist_rekey(vg_ocl_context_t *vocp)
{
    uint32_t *ocl_found_out;
    int i;
...
static int
vg_ocl_addrlist_check(vg_ocl_context_t *vocp, int slot)
{
    vg_exec_context_t *vxcp = &vocp->base;
    vg_context_t *vcp = vocp->base.vxc_vc;
    uint32_t *ocl_found_out;
    ...
    if (found_delta != 0xffffffff) {
        ...
        if (!memcmp(vxcp->vxc_binres + 1,
                ...
                20)) {
            res = 2;
            vg_output_match_console(vxcp, vxcp->vxc_binres, "pubkey");
        }
        if (res == 0) {
            fprintf(stderr, "Match idx: %d\n", ocl_found_out[1]);
            fprintf(stderr, "CPU hash: ");
...
static int
vg_ocl_addrlist_init(vg_ocl_context_t *vocp, vg_address_list_t *addrlist)
{
    cl_int ret;
    cl_mem clbuf;
    int i;

    if (!vg_ocl_create_kernel(vocp, 2, "hash_ec_point_search_list"))
        return 0;
    ...
    clbuf = clCreateBuffer(vocp->voc_oclctx,
                   ...
                   addrlist->count * HASH160_SIZE,
                   addrlist->addresses,
                   &ret);
    if (!clbuf) {
        fprintf(stderr, "clCreateBuffer(address_list): %s\n",
            vg_ocl_strerror(ret));
        fprintf(stderr, "Required: %lu MB\n",
            (addrlist->count * HASH160_SIZE) / (1024 * 1024));
        return 0;
    }
    ...
    for (i = 0; i < vocp->voc_nslots; i++) {
        ret = clSetKernelArg(vocp->voc_oclkernel[i][2],
                     ...
                     &clbuf);
        if (ret) {
            fprintf(stderr, "clSetKernelArg(address_list): %s\n",
                vg_ocl_strerror(ret));
            clReleaseMemObject(clbuf);
            return 0;
        }
    ...
    vg_ocl_kernel_int_arg(vocp, -1, 4, addrlist->count);

    vocp->voc_rekey_func = vg_ocl_addrlist_rekey;
    vocp->voc_check_func = vg_ocl_addrlist_check;
    return 1;
}
...
int
vg_ocl_context_set_address_list(vg_ocl_context_t *vocp,
                vg_address_list_t *addrlist)
{
    ...
    if (vcp->vc_verbose > 0) {
        fprintf(stderr, "Transferring %d addresses (%.2f MB) to GPU...\n",
            addrlist->count,
            (addrlist->count * HASH160_SIZE) / (1024.0 * 1024.0));
    }

    return vg_ocl_addrlist_init(vocp, addrlist);
}
